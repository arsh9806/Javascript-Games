        ...
        squares[i].onclick = function(){
            ...
            if(squares[index +7].classList.contains('taken')){
                if(currentPlayer ===1 ){
                    ...
                    squares[index].removeAttribute("onclick")
                ...
                //if the square below the current square is not occupied by the current player he/she is restricted
            }else alert("can't go there")
        }
    })(i)
//check the board for win or lose
function checkBoard(){
    //make const that shoes all winning arrays
    const winningArrays = [
    ...
    //now take the 4 values in each winngArrays and plug them into squares
    for (let y = 0; y < winningArrays.length; y++){
        ...
        //now check those arrays to see if they all have class of player two
...
//add an  eventListener to each square that willtrigger the checkboard function on click
